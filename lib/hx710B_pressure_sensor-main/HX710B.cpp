...
float HX710B::kPascal(){
	// 2^23 / 700kPa
    float value = (read_average())/11983725.714;//11983.72;//*RES) * 296.296 + 24.8148) ;//20 + 50);
    return value;
}

float HX710B::atm(){
    float value = kPascal()*9.86923E-9;
    return value;
}

float HX710B::mmHg(){
    float value = kPascal()*0.00750062;
    return value;
}

float HX710B::psi(){
    float value = kPascal()*0.000145038;
    return value;
}
